...
class MainScr(Screen):
    def __init__(self, **kwargs):
        ...
        hl = BoxLayout()
        txt = Label(text = 'Выбери экран', color = '#569a26')
        vl.add_widget(ScrButton(self, direction = 'down', goal = 'first', text = '1', color = '#af479e', pos_hint = {'left': 2}, size_hint = (.1, 2)))
        vl.add_widget(ScrButton(self, direction = 'left', goal = 'second', text = '2', color = '#32CD32', pos_hint = {'leupft': 4}, size_hint = (.2, 1)))
        vl.add_widget(ScrButton(self, direction = 'up', goal = 'third', text = '3', color = '#40E0D0', pos_hint = {'down': 1}, size_hint = (.3, 1)))
        vl.add_widget(ScrButton(self, direction = 'right', goal = 'fourth', text = '4', color = '#FF1493', pos_hint = {'right': 3}, size_hint = (.1, 3)))
        hl.add_widget(txt)
        hl.add_widget(vl)
        self.add_widget(hl)
...
class FourthScr(Screen):
    def __init__(self, **kwargs):
        super().__init__(**kwargs)
        vl = BoxLayout(oriantation = 'vertical', spacing = 8)
        a = 'START' + 'Выбор: 3' * 200
        ...
        self.label.blind(size = self.resize)
